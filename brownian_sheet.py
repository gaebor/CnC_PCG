...
def perlin(shape0, shape1, H=None):
    n = 2 ** math.ceil(numpy.log2(max(shape0, shape1)))
    octaves = 1 + int(numpy.log2(n))
    res = (1, 1)
    X = perlin2d.generate_fractal_noise_2d((n, n), res, octaves)[:shape0, :shape1]
    Y = perlin2d.generate_fractal_noise_2d((n, n), res, octaves)[:shape0, :shape1]
    return X, Y


def simple(n, m, H=0.5):
    """http://paulbourke.net/fractals/noise/"""
    X = standard_normal(size=(n, m)) + 1j * standard_normal(size=(n, m))
    r = numpy.sqrt(
...
def brownian(n, m, H=0.5):
    ...
    Z = standard_normal(size=(2 * (n - 1), 2 * (m - 1))) + 1j * standard_normal(
        size=(2 * (n - 1), 2 * (m - 1))
    )
    F = numpy.fft.fft2(lam * Z)[:n, :m]
    F -= F[0, 0]
    for i in [numpy.sqrt(2 * c2), 1j * numpy.sqrt(2 * c2)]:
        F += i * ((tx * standard_normal())[:, None] * (ty * standard_normal())[None, :])
...
The height difference determines the numbering of the contours through an
...
However, same level contours can meet because they are just level-lines of a surface,
...
|_|_
    ...
    if isinstance(minh, numpy.ndarray):
